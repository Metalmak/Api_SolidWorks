...
[ModelDoc::InsertCutSwept4](ModelDoc__InsertCutSwept4.htm)
...
|  |  |  |
| --- | --- | --- |
| Input: | (BOOLEAN) propagate | If TRUE, then the swept cut will propagate to the next edge, FALSE will cause the swept cut to occur only on the selected edge; to propagate to the next edge, the next edge must be tangent to the current edge |
| Input: | (BOOLEAN) alignment | If the curve used to sweep goes from on face to another, or from one edge to another, passing TRUE will cause the sweep to cut completely through the end faces of the cut; if you choose FALSE, then the swept cut will begin and end perpendicular to the sweep curve and, therefore, may not break through the two end faces of the body being cut |
