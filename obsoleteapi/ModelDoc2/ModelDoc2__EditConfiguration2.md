...
by ModelDoc2::EditConfiguration3.
...
| Return: | (VARIANT\_BOOL) retval | TRUE if the configuration is modified, FALSE if not |
...
| Output: | (VARIANT\_BOOL) retval | TRUE if the configuration is modified, FALSE if not |
| Return: | (HRESULT) status | S\_OK if successful |

Remarks

The flags argument allows additional control of the created configuration.
It can take these values from the ConfigurationFlags\_e enumeration in
swoptions.h:
