...
2018\samples\tutorial\weldments\weldment\_box2.sldprt",
swDocPART, swOpenDocOptions\_Silent, "", errors, warnings)
...
    Set Study = StudyMngr.**CreateNewStudy3**("frame",
...
        Set BeamBody = BeamMgr.**GetBeamBodyAt**(j,
errCode)
...
    Debug.Print "  Pinball radius: " & Joints.**PinBallRadius**
\* 0.001
...
swsLinearUnitMillimeters, ElementSize, Tolerance

    ' Select structural members for mesh
control
    Debug.Print ""
    Debug.Print ("Mesh control...")

    status = swModelDocExt.**SelectByID2**("Square
tube 30 X 30 X 2.6(1)[4]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(0) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Square
tube 30 X 30 X 2.6(1)[3]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(1) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Square
tube 30 X 30 X 2.6(1)[2]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(2) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Square
tube 30 X 30 X 2.6(1)[1]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(3) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Square
tube 30 X 30 X 2.6(1)[8]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(4) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Square
tube 30 X 30 X 2.6(1)[6]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(5) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Sb
beam 80 X 6(1)[2]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(6) = selEntity
    Set selEntity = Nothing

    status = swModelDocExt.**SelectByID2**("Sb
beam 80 X 6(1)[1]", "SOLIDBODY", 0, 0, 0, False, 0, Nothing, 0)
    Set selEntity = swSelMgr.**GetSelectedObject6**(1, -1)
    Set selEntities(7) = selEntity
    Set selEntity = Nothing

    Set MeshControl = Mesh.**ApplyMeshControl**(selEntities,
errCode)
    MeshControl.**MeshControlBeginEdit**
    MeshControl.**BeamSelected2** = 0
    MeshControl.**MinimumElementSize\_BlendedCurved** = 4
    MeshControl.**MinNumOfElementsPerCircle\_BlendedCurved** =
8
    MeshControl.**MeshControlEndEdit**
    Debug.Print "  Name of mesh control: " & MeshControl.**Name**
    Debug.Print "  Beam selected? (True = yes; False = no) " &
MeshControl.**BeamSelected2**
...
i)/(layer i-1): " & MeshControl.**Ratio**
    Debug.Print "  State of mesh control (0 = suppressed; 1
= not suppressed): " & MeshControl.**State**
    Debug.Print "  Minimum element size for boundaries with
highest curvature: " & MeshControl.**MinimumElementSizeForBlendedCurveMesher**
    Debug.Print "  Minimum number of elements in a circle to
determine the maximum angle: " & MeshControl.**MinNumOfElementsPerCircleForBlendedCurveMesher**

    Debug.Print "  Number of mesh
controls: " & Mesh.**MeshControlCount**
