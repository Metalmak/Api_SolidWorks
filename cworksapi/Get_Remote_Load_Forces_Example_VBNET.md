...
        'of
the expected value. If the actual is not within the tolerance
        'range,
an error message is reported with {sMessage}
        'as
title, and the function returns false. Otherwise, the function
        'returns
...
        iSolverType =
swsSolverType\_e.swsSolverTypeFFEPlus      'Solver
type
...
        Part = swApp.**OpenDoc6**(sModelName, swDocumentTypes\_e.swDocPART,
1, "",
...
Then
            'Yes,
set the minimum element size equal to the maximum element size
...
Nothing,
(DispArray1), 0, errCode)

        Debug.Print("Remote load forces:")
        Debug.Print("   Remote load force
x-component: " & Force(0))
        Debug.Print("   Remote load force
y-component: " & Force(1))
        Debug.Print("   Remote load force
z-component: " & Force(2))
        Debug.Print("   Resultant remote
load force: " & Force(3))

        If
errCode <> 0 Then
ErrorMsg(SwApp, "Failed to get remote
load force result") :
GoTo Lastline
...
        If
VerifyTolerance(SwApp, UResMax, CDbl(FRES),
