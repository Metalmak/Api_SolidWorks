...
    'Model names
    sModelName = "**C:\Users\Public\Documents\SOLIDWORKS\SOLIDWORKS 2018\samples\tutorial\api\beam\_boltconnection.sldasm**"
...
    iSolverType = swsSolverTypeDirectSparse 'Solver
type
...
swDocASSEMBLY, 1, "", longstatus, longwarnings)
...
        Call CWMesh.**GetDefaultElementSizeAndTolerance**(iMeshUnit,
el\_hold, tl\_hold)
...
        'Using the curvature-based mesher; set the min element
size equal to the max element size
...
    If VerifyLong(SwApp, swsMeshStateExistsAndCurrent, CWMesh.**MeshState**,
...
"Counterbore with Nut-1", 0, errCode)

    If errCode <> 0 Then ErrorMsg SwApp,
"Failed to get connector force result": GoTo Lastline
...
    UResMax = 0.467
    Tol1 = 0.1
    If VerifyTolerance(SwApp, UResMax, CDbl(Force(11)), Tol1,
"Resultant bending moment is beyond tolerance") = False Then GoTo Lastline
