...
        Part = swApp.**OpenDoc6**("C:\Users\Public\Documents\SOLIDWORKS\SOLIDWORKS 2019\samples\Simulation Examples\Tutor1.SLDPRT ", swDocumentTypes\_e.swDocPART, swOpenDocOptions\_e.swOpenDocOptions\_Silent, "", longstatus, longwarnings)
...
        ' Add
default static study results plot

errCode = ActDoc.**AddDefaultStaticStudyPlot**(swsDefaultStaticResultTypes\_e.swsStaticResultElementalStrain,
swsStaticResultElementalStrainComponentTypes\_e.swsStaticElementalStrain\_ENERGY)

' Set default plot boundary color
        errCode = ActDoc.**SetSimulationOptionDefaultPlotsBoundaryColorInRGB**(swsSimulationOptionDefaultPlotsBoundaryColorInRGBBoundaryOption\_e.swsSimulationOptionDefaultPlotsBoundaryColorInRGBBoundaryOption\_ModelOrMesh, 0, 0, 255)
        errCode = ActDoc.**SetSimulationOptionDefaultPlotsBoundaryColorInRGB**(swsSimulationOptionDefaultPlotsBoundaryColorInRGBBoundaryOption\_e.swsSimulationOptionDefaultPlotsBoundaryColorInRGBBoundaryOption\_TranslucentSingleColor, 255, 0, 0)

        ' Get default plot model or mesh boundary color
        Dim colorObj As Object
        colorObj = ActDoc.**GetSimulationOptionDefaultPlotsBoundaryColorInRGB**(swsSimulationOptionDefaultPlotsBoundaryColorInRGBBoundaryOption\_e.swsSimulationOptionDefaultPlotsBoundaryColorInRGBBoundaryOption\_ModelOrMesh, errCode)
...
        Study = StudyMngr.**CreateNewStudy3**("Static\_solid", swsAnalysisStudyType\_e.swsAnalysisStudyTypeStatic, swsMeshType\_e.swsMeshTypeSolid, errCode)
...
        ' Fourth selection is the direction
reference face
...
        If LBCMgr Is Nothing Then ErrorMsg(swApp, "No CWLoadsAndRestraintsManager
object")
...
        CWPressure.**CoordSystemType** = swsCoordinateType\_e.swsCoordinateTypeCartesian
        CWPressure.**Equation** = ".4\*""x"" + .8\* ""y"" + .6\* ""z"""
        errCode = CWPressure.**PressureEndEdit**
        If errCode <> 0 Then ErrorMsg(swApp, "Pressure not set with error code as defined in swsPressureEndEditError\_e: " & errCode)
...
        results = CWResult.**GetStrainForEntities3**(True, CType(swsStrainComponent\_e.swsStrainComponentESTRN, Integer), 1, Nothing, dispWrapper, status,
0, False, errCode)
...
        results = CWResult.**GetStressForEntities3**(True, CType(swsStressComponent\_e.swsStressComponentVON, Integer), 1, Nothing, dispWrapper, swsStrengthUnit\_e.swsStrengthUnitPascal, status,
0, False, errCode)
        For i = 0 To UBound(results)
            resultsString = Convert.ToString(results(i))
            Debug.Print("     " & resultsString)
...
swsStaticResultNodalStrainComponentTypes\_e.swsStaticNodalStrain\_ESTRN,
swsUnit\_e.swsUnitSI,  False,
errorCode)
errorCode = plot.**ActivatePlot**errorCode = plot.**GetPlotName**(plotName)
...
swsDeformType\_e.swsAutomatic, 0,
True)
...
"has minimum strain value: " & results(1))
Debug.Print("Node
" & results(2) &
...
Print("Number
of plots for these results: " & CWResult.**GetPlotCount**)
For
Each Name
In CWResult.**GetPlotNames**
...
vDispOptions = CWResult.**GetPlotDisplayOptions**(Name,
errorCode)
Debug.Print("")
Debug.Print("Plot
display options:")
...
Debug.Print("
Chart number format as defined in
swsColorChartNumberFormatOptionValue\_e: "  &
...
vPlotSettings = CWResult.**GetPlotSettings**(Name,
errorCode)
