...
confusing. For example, p-curves, as in IModeler::CreatePCurve, are b-splines
...
* ICurve::GetPCurveParams
  gets the polynomial coefficients for the segments of a b-spline for the
  input curve.
* ICurve::GetPCurveParams2
  is a hybrid of ICurve::GetBCurveParams5 and ICurve::GetPCurveParms. You
...
* ISketchManager::CreateSpline2 creates a b-spline using the input points as
  through points, or interpolation points, with some control over the curve’s
  end conditions.
* IModelDoc2::SketchSpline is an attempt to simulate the way you create a b-spline
  in the SOLIDWORKS user interface, one point at a time.
* IFeatureManager::MakeStyledCurves2 fits a b-spline that approximates a
  composite curve of sketch segments.

**What b-spline points for the curve does ICurve::GetSplinePts return?**

ICurve::GetSplinePts gets a spline's through/interpolation points, i.e., the end
