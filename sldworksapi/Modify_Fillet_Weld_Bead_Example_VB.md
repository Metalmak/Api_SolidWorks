...
    boolstatus = swModelDocExt.**SelectByID2**("Fillet Bead1",
"BODYFEATURE", 0, 0, 0, False, 0, Nothing, 0)
    Set swFeat = swSelMgr.**GetSelectedObject6**(1, -1)
    Set swWeldBead = swFeat.**GetDefinition**

    'Roll back to the feature just before
Fillet Bead1
...
swWeldBeadType\_e: " & bdTy

    Debug.Print "  Propagate the weld
bead along the tangent? " & swWeldBead.**TangentPropagation**
    Debug.Print "  Apply weld bead to both sides of
intersecting faces? " & swWeldBead.**UseOtherSide**

    'Get Fillet Bead1 faces
    swWeldBead.**GetFaces** swWeldBeadArrowSide, set1, faceVar

    For i = LBound(faceVar) To UBound(faceVar)
        faceVar(i).**Select4** True,
swSelData
    Next i
...
    ve = swWeldBead.**GetVirtualEdges**(False,
swWeldBeadArrowSide)

    For i = LBound(ve) To UBound(ve)
        boolstatus = ve(i).**Select4**(True,
swSelData)
...
    swWeldBead.**BeadLength**(swWeldBeadArrowSide) = bdlen \*
0.5
    swWeldBead.**BeadPitch**(swWeldBeadArrowSide) = bdPitch \*
0.5
