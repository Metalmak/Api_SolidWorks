...
// 2. Verify that **c:\test**, the folderwhereto save
...
            swExportPDFData = (ExportPdfData)swApp.**GetExportFileData**((int)swExportDataFileType\_e.swExportPdfData);
...
to a PDF file
            boolstatus =
swExportPDFData.**SetSheets**((int)swExportDataSheetsToExport\_e.swExportData\_ExportSpecifiedSheets,
(arrObjIn));
            swExportPDFData.**ViewPdfAfterSaving** =
true;
            boolstatus = swModExt.**SaveAs**(filename, (int)swSaveAsVersion\_e.swSaveAsCurrentVersion,
