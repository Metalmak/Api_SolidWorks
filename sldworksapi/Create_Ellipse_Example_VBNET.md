...
the  major axis, and run the macro again.")
            Exit
Sub
        End
If
...
        ' Ensure that the minor axis is
less than the major axis so that
        '
...
SkEllipse As
SketchEllipse
        SkEllipse = swModel.**SketchManager**.**CreateEllipse**(CtrPt.**X**,
...
        If
swSketch.GetEllipseCount = 0 Then
            MsgBox("An
...
        ' Return the tipping angle, theta;
avoid divide by zero
        If
swMajPt.**X** <> swCtrPt.**X** Then
            theta = Atan((swMajPt.Y -
...
        ' NOTE: The coefficients of x and y
in this
        '
