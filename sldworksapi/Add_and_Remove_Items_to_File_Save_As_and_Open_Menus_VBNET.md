...
' 3. Copy and paste [this code](#SwAddin) into **SwAddin.vb** of
your VB.NET project.
' 4. Select **Project > SwVBAddin1 Properties > Debug**.
' 5. Select **Start external program** and type the path to **sldworks.exe**.
Include
...
<Guid("a3ddcdce-c8a8-47c2-8561-919a24285286")>
\_
    <ComVisible(True)>
\_
    <SwAddin( \_
        Description:="SwVBAddin1
third-party pop-up menus", \_
        Title:="SwVBAddin1",
\_
        LoadAtStartup:=True
\_
...
        Dim
SWattr As
SwAddinAttribute = Nothing

        attributes =
System.Attribute.GetCustomAttributes(GetType(SwAddin),
GetType(SwAddinAttribute))
...
        Dim
addinkey As
Microsoft.Win32.RegistryKey = hklm.CreateSubKey(keyname)
...
        bRet = iSwApp.**AddFileSaveAsItem2**(addinID, "XXX\_FileSave", "XXX file (\*.xxx)", "xxx", swDocumentTypes\_e.swDocPART)
...
        bRet = iSwApp.**RemoveFileSaveAsItem2**(addinID, "XXX\_FileSave", "XXX file (\*.xxx)", "xxx", swDocumentTypes\_e.swDocPART)
...
docTypes() As
Integer =
{swDocumentTypes\_e.swDocASSEMBLY, \_
                                       swDocumentTypes\_e.swDocDRAWING, \_
...
"",
-1)
        cmdGroup.LargeIconList = iBmp.**CreateFileFromResourceBitmap**("SwVBAddin1.ToolbarLarge.bmp",
thisAssembly)
        cmdGroup.SmallIconList = iBmp.**CreateFileFromResourceBitmap**("SwVBAddin1.ToolbarSmall.bmp",
thisAssembly)
        cmdGroup.LargeMainIcon = iBmp.**CreateFileFromResourceBitmap**("SwVBAddin1.MainIconLarge.bmp",
thisAssembly)
        cmdGroup.SmallMainIcon = iBmp.**CreateFileFromResourceBitmap**("SwVBAddin1.MainIconSmall.bmp",
thisAssembly)
...
            Dim
cmdTab As
ICommandTab = iCmdMgr.**GetCommandTab**(docType, Title)
            Dim
bResult As
Boolean
...
Nothing
Then
                cmdTab = iCmdMgr.**AddCommandTab**(docType,
Title)

                Dim
cmdBox As
CommandTabBox = cmdTab.**AddCommandTabBox**
...
        frame = SwApp.**Frame**()
        resultCode = frame.**AddMenuPopupIcon2**(swDocumentTypes\_e.swDocPART,
swSelectType\_e.swSelFACES, "third-party
context-sensitive", addinID,
"CSCallbackFunction",
"CSEnable",
"",
...
        ' add a menu break at the top of
the shortcut menu
        resultCode = SwApp.**AddItemToThirdPartyPopupMenu2**(registerID,
CInt(swDocumentTypes\_e.swDocPART),
"Menu Break",
addinID, "",
"",
"",
"",
"",
CInt(swMenuItemType\_e.swMenuItemType\_Break))
        ' add a couple of items to the
shortcut menu
        resultCode = SwApp.**AddItemToThirdPartyPopupMenu2**(registerID,
CInt(swDocumentTypes\_e.swDocPART),
"Test1",
...
"",
"Test1",
cmdGroup.**SmallMainIcon**, CInt(swMenuItemType\_e.swMenuItemType\_Default))
        resultCode = SwApp.**AddItemToThirdPartyPopupMenu2**(registerID,
CInt(swDocumentTypes\_e.swDocPART),
"Test4",
addinID, "TestCallback",
...
"",
"",
CInt(swMenuItemType\_e.swMenuItemType\_Separator))
        ' add another item to the shortcut
menu
        resultCode = SwApp.**AddItemToThirdPartyPopupMenu2**(registerID,
CInt(swDocumentTypes\_e.swDocPART),
"Test5",
...
        ' add an icon to a menu bar of the
shortcut menu
        resultCode = SwApp.**AddItemToThirdPartyPopupMenu2**(registerID,
CInt(swDocumentTypes\_e.swDocPART),
"",
...
AddressOf
Me.SldWorks\_ActiveDocChangeNotify
        AddHandler
iSwApp.**DocumentLoadNotify2**,
AddressOf
...
AddressOf
Me.SldWorks\_ActiveModelDocChangeNotify
        AddHandler
iSwApp.**FileOpenPostNotify**, AddressOf
Me.SldWorks\_FileOpenPostNotify
        AddHandler iSwApp.**InterfaceBrightnessThemeChangeNotify**,
AddressOf Me.SldWorks\_InterfaceBrightnessChangeThemeNotify
...
"Event Handlers"
    Function
SldWorks\_ActiveDocChangeNotify() As
Integer
        'TODO:
...
    Function
SldWorks\_DocumentLoadNotify2(ByVal
docTitle As
String,
ByVal
docPath As
...
    Function
SldWorks\_FileNewNotify2(ByVal
newDoc As
Object,
ByVal
doctype As
...
    Function
SldWorks\_ActiveModelDocChangeNotify() As
Integer
        'TODO:
Add your implementation here
    End
...
    Function
SldWorks\_FileOpenPostNotify(ByVal
FileName As
String)
As
Integer
...
    Function
SldWorks\_InterfaceBrightnessChangeThemeNotify(ByVal
Theme As
Integer,
...
        partTemplate = iSwApp.**GetUserPreferenceStringValue**(swUserPreferenceStringValue\_e.swDefaultTemplatePart)
