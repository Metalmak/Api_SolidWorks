...
    swApp.**GetBuildNumbers2** strBaseVersion,
strCurrentVersion, strHotFixes
...
        Set swDocument = swApp.**OpenDoc6**(strFileName,
swDocPART, lOptions, "", lErrors, lWarnings)
        Debug.Print "lErrors = " & lErrors

        ' Starting with SW2012 SP5, loading
future file versions is supported,
        ' so the future version error no longer occurs.
        Debug.Print "  future version
error = " & ((lErrors And swFileLoadError\_e.swFutureVersion) =
swFileLoadError\_e.swFutureVersion)
...
    ' The version history of a future version
...
Private Sub PrintVersionHistory(ByVal vVersionHistory As
Variant)
...
        strDateCodes =
Replace(vSplitResults(1), "]", "")
        vSplitResults = Split(strDateCodes,
",")
        Debug.Print "
format version = " & strFormatVersion
...
Private Function BlockingState2String(ByVal nBlockingState As
SwConst.swBlockingStates\_e) As String

    Select Case (nBlockingState)
        Case swBlockingStates\_e.swEditorBlock
