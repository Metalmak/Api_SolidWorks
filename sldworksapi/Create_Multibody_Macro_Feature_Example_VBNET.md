...
'  1. Contact SOLIDWORKS API Support to obtain **API: C#\VB.NET Add-ins to Create a
Macro Feature**.
'  2. Copy **InsertMacroFeature VB.NET Addin Mods for 64
bit.zip** to a local drive.
'  3. Unzip the file.
...
'  7. Replace Region I**SwComFeature Implementation** with [this](#Region).
'  8. Replace **AddMacroFeature()** with [this](#AddMacroFeature).
'  9. Click **Project > SwVBAddinTest Properties > Application** and
change
'     T**arget framework** to **.NET FrameWork 4**.
' 10. Click **Compile > Build Events** and replace the code in **Post-build
event**
'     **command line** with [this](#Post).
...
'     step 1.
' 12. Open SOLIDWORKS.
' 13. Open *public\_documents***\samples\tutorial\multibody\multi\_local.sldprt.**'
' Postconditions:
'  1. Click **OK** in each message box.
'  2. Displays  **VB Addin** on the Tools menu.
'  3. Fires the ISldWorks\_ActiveDocChangeNotify event and

'     inserts the macro feature, **MacroFeature1**, in
the
'     FeatureManager design tree, which:
'     \* Consumes the part's solid bodies, **Fillet5** and **Fillet6**.
'     \* Creates two solid bodies, **MacroFeature1[1]** and
**MacroFeature1[2]**.
...
        vBodies = moddoc.**GetBodies2**(swBodyType\_e.swAllBodies, False)

        'Create the macro feature
        MacroFeature = FeatMgr.**InsertMacroFeature3**("MacroFeature", "swVBAddinTest.SwAddin", Nothing, \_
            Nothing, Nothing, Nothing, Nothing, Nothing, vBodies, Nothing, swMacroFeatureOptions\_e.swMacroFeatureByDefault)
...
IF EXIST
"$(TargetDir)$(TargetName).bmp" (GOTO REGISTRATION)
