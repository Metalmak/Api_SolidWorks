...
' Preconditions:
' 1.
Open an assembly document with at least one component.
...
swSelectType\_e.swSelBODYFEATURES)
...
type as defined in swSelectType\_e: " &
swFeature.**GetType**)
        Debug.Assert(swFeature.**GetType** =
swSelectType\_e.swSelBODYFEATURES)
...
        ' Select the feature through the
Entity interface
        bValue = swEntity.**Select4**(False,
Nothing)

        ' Print the type of the selected
object
        Debug.Print("Selected
object type as defined in swSelectType\_e: "
& swModel.**SelectionManager**.**GetSelectedObjectType3**(1, -1))
        Debug.Assert(swModel.**SelectionManager**.**GetSelectedObjectType3**(1,
-1) = swSelectType\_e.swSelBODYFEATURES)
