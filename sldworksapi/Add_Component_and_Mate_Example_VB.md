...
'---------------------------------------------------------------------------
' Preconditions:
' 1. Copy and paste [this code](#Main0) in the main module.
' 2. Click **Insert > Class module** and copy and paste [this
code](#Class) in the class
'    module.
' 3. Verify that these documents exist in *public\_documents***\samples\tutorial\toolbox:
'** \* **lens\_mount.sldasm
'** \* **camtest.sldprt**
...
    ' Use AssemblyName when mating the
...
cannot be found; display error messages
    If warnings = swFileLoadWarning\_ReadOnly Then
...
    ' Activate the assembly so that you can
...
swAddComponentConfigOptions\_CurrentSelectedConfig, "", False, "", -1, -1, -1)
...
    ' Add the mate
    Set matefeature = swAssy.**AddMate5**(swMateCOINCIDENT,
swMateAlignALIGNED, False, 0, 0, 0, 0, 0, 0, 0, 0, False, False, 0, mateError)
...
Private Function swAssy\_**AddItemNotify**(ByVal EntityType As
Long, ByVal itemName As String) As Long
    Debug.Print "Component added: " & itemName
End Function

Private Function swAssy\_**RenameItemNotify**(ByVal
