...
Sub GetPathNames(swApp As SldWorks.SldWorks, ByVal
swBomFeature As SldWorks.BomFeature)
    Dim vConfigurations
As Variant
...
    ' For now assume we have only 1 table
annotation.
...
    If (Not (swTableAnnotation.TitleVisible =
False)) Then
        lStartRow = 2
    End If
...
vModelPathNames = swTableAnnotation.**GetModelPathNames**(lRow, strItemNumber,
strPartNumber)
