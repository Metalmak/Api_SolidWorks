...
// 4. Examine the
Immediate window and graphics area.
...
swView = (View)swSelectionMgr.GetSelectedObject6(1,
-1);
...
Debug.Print("Current
display mode as defined by swDisplayMode\_e:
" + displayMode);
...
Debug.Print("Display
edges when the view is in shaded mode?
" + EdgesMode);
...
cosmetic thread dislay quality? " + cThreadQuality);

// Reset display mode:
Do not use parent view's display mode, shaded mode, draft quality, no edges
displayed, precision cosmetic threads

swView.**SetDisplayMode4**(false,
3,
...
Debug.Print("Using
parent view mode?  "
+ swView.**GetUseParentDisplayMode**());
...
Debug.Print("Faceted
(draft quality)?  "
+ swView.**GetFacettedHlrDisplay**());

Debug.Print("Display
edges when view is in shaded mode?
" + swView.**GetDisplayEdgesInShadedMode**());

Debug.Print("Precision
cosmetic thread dislay quality? " + swView.**GetCThreadQuality**());
