...
' Postconditions: When the macro stops, press F5 five more
times to create
...
    vSkLines = Part.**SketchManager**.**CreateCornerRectangle**(-4.72149072628614E-02,
2.51029484316118E-02, 0, 6.72972660081509E-02, -4.56125914055245E-02, 0)
    Part.**ClearSelection2** True
    Part.**SketchManager**.**InsertSketch** True
...
        bValue = Part.**Extension**.**SelectByID2**("Sketch1",
...
bValue = Part.**Extension**.**SelectByID2**("", "VERTEX",
-0.05999251028807, 0.05489880658436, 0, True, 0, Nothing, 0)

        End Select

        Set myFeature =
Part.**FeatureManager**.**FeatureExtrusionThin2**(True, False, False,
...
True, nStartCondition, dStartOffset, bFlipOffset)

        Debug.Print
myFeature.**Name** & " [" & myFeature.**GetTypeName** & "]"

        Set
swExtrusionData = myFeature.**GetDefinition**

        bValue =
swExtrusionData.**AccessSelections**(Part, Nothing)

        If (swExtrusionData.**IsThinFeature**
= False) Then
            Debug.Print
"ERROR: Must be a thin feature."
        End If

        Select Case
swExtrusionData.**FromType**

Case SwConst.swExtrudeFrom\_e.swExtrudeFrom\_SketchPlane

Debug.Print "  from: sketchplane"

Case SwConst.swExtrudeFrom\_e.swExtrudeFrom\_Offset
...
Case SwConst.swExtrudeFrom\_e.swExtrudeFrom\_SurfaceFacePlane

Debug.Print "  from: surface"

Case SwConst.swExtrudeFrom\_e.swExtrudeFrom\_Vertex
...
select it again
        bValue = myFeature.**Select2**(True,
0)

        bValue = Part.**Extension**.**DeleteSelection2**(swDelete\_Children)
