...
        swModel = swApp.**OpenDoc6**(fileName, swDocumentTypes\_e.swDocDRAWING, swOpenDocOptions\_e.swOpenDocOptions\_Silent, "", errors, warnings)
        swDrawing = swModel
        swSelMgr = swModel.**SelectionManager**
        swSelData = swSelMgr.**CreateSelectData**
        swApp.**ActivateDoc3**("replaceview - Sheet1", False, swRebuildOnActivation\_e.swDontRebuildActiveDoc, errors)
...
        swView = swDrawing.**CreateDetailViewAt4**(0.22305342706156, 0.0762140266484527, 0, swDetViewStyle\_e.swDetViewSTANDARD, 1, 1, "A", swDetCircleShowType\_e.swDetCircleCIRCLE, True,
True, False, 5)
...
as Double
        Dim ypos
as Double    swDetailCircle.**GetLabelPosition**(xpos,
ypos)
Debug.Print("  Label
X position: " & xpos)
...
outline? False")
            If
swDetailCircle.**JaggedOutline** Then

swDetailCircle.**ShapeIntensity** = 2
                Debug.Print("  Jagged
outline and shape intensity? True and 2")
            End If
        End If
