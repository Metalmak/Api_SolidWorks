...
namespace
GetOriginalBodyFromPattern\_CSharp.csproj
{
    partial
class
SolidWorksMacro
...
            swBody = (Body2)swSm.**GetSelectedObject6**(1,
-1);
            Debug.Print("Number
of edges in pattern body: " + swBody.**GetEdgeCount**());
...
                swOriBody = swBody.**GetOriginalPatternedBody**(out
swMathTrans);
