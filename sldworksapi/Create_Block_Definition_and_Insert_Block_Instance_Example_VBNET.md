...
    Sub PositionNote(ByVal swApp As SldWorks, ByVal swModel As ModelDoc2, ByVal swSkNote As Note, ByVal X\_pos As Double, ByVal Y\_pos As Double, ByVal Z\_pos As Double)
...
        bRet = swSkNote.**SetBalloon**(swBalloonStyle\_e.swBS\_None, swBalloonFit\_e.swBF\_Tightest)

        nRetVal = swAnn.**SetLeader3**(swLeaderStyle\_e.swBENT, swLeaderSide\_e.swLS\_SMART, True, False, False, False)

        bRet = swAnn.**SetPosition2**(X\_pos, Y\_pos, Z\_pos)
