...
namespace
GetBrokenOutSections\_CSharp.csproj
{
    partial
class
SolidWorksMacro
...
            for
(I = varSheet.GetLowerBound(0); I <= varSheet.GetUpperBound(0); I++)
            {
                varViews = (object[])varSheet[I];
                // skip the sheet view
                for
(J = 1; J <= varViews.GetUpperBound(0); J++)
                {
                    View
swView = default(View);
...
" + swView.Name);
                    Debug.Print("  Number
of broken-out sections in this view: "
+ swView.**GetBreakOutSectionCount**());
                    varBS = (object[])swView.**GetBreakOutSections**();
                    if
((!((varBS == null))))
                    {
                        for
(K = varBS.GetLowerBound(0); K <= varBS.GetUpperBound(0); K++)
                        {
...
                            // get and
select depth reference, if set
                            if
((swDepthRef == null))
                            {
                                Debug.Print("              Depth
of exposure in meters: " + swFeatData.**Depth**);
                            }
                            else
                            {
...
            {
                Debug.Print("              Number
of sketch segments: " + swFeatData.**GetSketchSegmentCount**());
            }
        }
        public
void
...
on broken-out section: " + inFeat.Name);
...
varskSeg.GetUpperBound(0); segCtr++)
                    {
                        SketchSegment
swSeg = default(SketchSegment);
...
                                break;
                            case
(int)swSketchSegments\_e.swSketchARC:
                                SketchArc
swSegArc = default(SketchArc);
                                swSegArc = (SketchArc)swSeg;
                                Debug.Print("                  Segment
type: Arc");
                                Debug.Print("                      Old
radius: " + swSegArc.GetRadius());
                                double
newRad = 0;
...
                    }
                }
                inFeat.ModifyDefinition(swFeatData, swDoc,
null);
