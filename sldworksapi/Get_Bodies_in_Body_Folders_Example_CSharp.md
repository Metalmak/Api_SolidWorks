...
            if
(FeatType == "SolidBodyFolder"
| FeatType == "SurfaceBodyFolder"
| FeatType == "CutListFolder"
| FeatType == "SubWeldFolder"
| FeatType == "SubAtomFolder")
            {
                IsBodyFolder = true;
            }
...
+ "," +
FeatType + ","
+ BodyFolderType[BodyFolderTypeE] + "
type " + BodyFolderTypeE);
...
                {
                    for
(i = vBodies.GetLowerBound(0); i <= vBodies.GetUpperBound(0); i++)
                    {
                        Body2
Body = default(Body2);
...
                Feature
FeatureFromBodyFolder = default(Feature);
                FeatureFromBodyFolder = BodyFolder.**GetFeature**();

                if
((!object.ReferenceEquals(FeatureFromBodyFolder,
thisFeat)))
                {
...
        {
            Feature
curFeat = default(Feature);
            curFeat = thisFeat;

            while
((curFeat != null))
            {
                DoTheWork(curFeat); //Do
the thing that we are doing this feature traversal for

                Feature
subfeat = default(Feature);
                subfeat = (Feature)curFeat.**GetFirstSubFeature**();

                while
((subfeat != null))
                {
                    TraverseFeatures(subfeat,
false);
                    Feature
nextSubFeat = default(Feature);
