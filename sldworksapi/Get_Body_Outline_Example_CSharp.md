...
namespace
GetBodyOutline2\_CSharp.csproj
{
    partial
class
SolidWorksMacro
...
            bVar = (object[])swPart.**GetBodies2**((int)swBodyType\_e.swSolidBody,
false);
            int
bdycnt = bVar.GetLength(0);
...
            direction
userDirection = default(direction);
            userDirection = direction.Y;

            if
(userDirection == direction.X)
            {
                dirArr[0] = 1;
            }
            else
...
outline);
            Object[]
crvOut = (Object[])curveOut;

            //Open a 3D sketch in the part
document
            swModel.**Insert3DSketch**();

            //Using the end conditions of
the curves, create a 2D sketch of each curve
            Curve[]
vCurve = null;
            int
newCt = 0;
...
(i = 0; i <= nCt - 1; i++)
            {
                ((Curve)crvOut[i]).**GetEndParams**(out
s, out e,
out
isClosed, out
isPer);
                if
(((Curve)crvOut[i]).**GetLength3**(s,
e) > 1E-05)
                {
                    Array.Resize(ref
vCurve, newCt + 1);
...
                if
(userDirection == direction.X
| userDirection == direction.Xminus)
                {
                    sPoints[i \* 3] = 0;
                    ePoints[i \* 3] = 0;
                }
...
if (userDirection
== direction.Y
| userDirection == direction.Yminus)
                {
                    sPoints[i \* 3 + 1] = 0;
                    ePoints[i \* 3 + 1] = 0;
                }
...
if (userDirection
== direction.Z
| userDirection == direction.Zminus)
                {
                    sPoints[i \* 3 + 2] = 0;
                    ePoints[i \* 3 + 2] = 0;
                }
...
(i = 0; i <= (newCt \* 3) - 1; i += 3)
            {
                swModel.**CreateLine2**(sPoints[i], sPoints[i + 1],
sPoints[i + 2], ePoints[i], ePoints[i + 1], ePoints[i + 2]);
            }
