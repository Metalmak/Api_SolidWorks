...
|  |
| --- |
| [SolidWorks.Interop.sldworks Namespace](SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks_namespace.html) > [IFace2 Interface](SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IFace2.html) : GetTessTriStripNorms Method (IFace2) |
...
See Face2::GetTessTriStripNorms.
...
* NumStrips = Number of strips on a particular face* VertexPerStrip = Array containing the number of vertex points on each face strip* Normals = Array of X,Y,Z normal components for each vertex on each face strip
