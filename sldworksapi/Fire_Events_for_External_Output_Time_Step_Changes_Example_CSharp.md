...
//    b. Open *public\_documents***\samples\tutorial\api\simple-block.sldasm.**
//    c.
Click **MotionStudy 1** and select
**Motion Analysis** in the
...
browse to  *//    install\_dir***\api\redist** > click
...
namespace
MotionExternalForceMotorEventsCSharp.csproj
{
...
            SetForceExternalFlag("Force1",
true);

            // Set feature LinearMotor1 as external
motor
            SetMotorExternalFlag("LinearMotor1",
true);
...
        private
void SetForceExternalFlag(string
featureName, bool setFlag)
        {
            // Set forces to external; i.e., listen
for events
...
outputWrite = new
System.IO.StreamWriter("c:\\temp\\ForceAndMotorValuesCSharp.txt",
true);
...
swModel, null);
                        outputWrite.WriteLine("Feature:
" + swFeature.**Name**);
                        outputWrite.WriteLine("  Listen
for events: " + swSimulationFeatureData.**ExternalState**);
...
        private
void SetMotorExternalFlag(string
featureName, bool setFlag)
        {
            // Set motors to external; i.e., listen
for events
...
                if (simTypeName
== "AEMLinearMotor" |
simTypeName == "AEMRotationalMotor")
                {
                    if (swFeature.**Name**
== featureName)
                    {
...
            outputWrite.WriteLine("Name: "
+ Name);
            outputWrite.WriteLine("  Value: "
+ value.ToString());
...
this.motionStudy\_ForceOutputTimeStepChangeNotify;
            motionstudy.MotorOutputTimeStepChangeNotify +=
this.motionStudy\_MotorOutputTimeStepChangeNotify;
            motionstudy.OutputTimeStepChangeNotify +=
this.motionStudy\_OutputTimeStepChangeNotify;
            motionstudy.StartCalculateNotify += this.motionStudy\_StartCalculateNotify;
            motionstudy.StopCalculateNotify += this.motionStudy\_StopCalculateNotify;
        }

        public
int motionStudy\_**ForceOutputTimeStepChangeNotify**(double
Time, object ForceNames,
object Disp,
object Velocity,
object Acceleration,
...
        {
            // Fire event for every external force or
torque output time step change
            MotionStudyProperties
swMotionStudyProperties = default(MotionStudyProperties);
            int i = 0;
            double
frameRate = 0;
            int numForces =
...
            swMotionStudyProperties = (MotionStudyProperties)motionstudy.**GetProperties**((int)swMotionStudyType\_e.swMotionStudyTypeCosmosMotion);
...
                if ((arrForceNames[i].ToString()
== "Force1") || (arrForceNames[i].ToString()
== "Force2") || (arrForceNames[i].ToString()
== "Force3"))
                {
                    A = 0.8;
                    w = 2 \* pi / 1.5;
...
if ((arrForceNames[i].ToString()
== "Torque1" |
arrForceNames[i].ToString() == "Torque2"
| arrForceNames[i].ToString() == "Torque3"))
                {
                    A = 0.005;
                    w = pi;
                }
                else
if ((arrForceNames[i].ToString()
== "Action Reaction Force1"
| arrForceNames[i].ToString() == "Action Reaction
Force2" | arrForceNames[i].ToString() ==
"Action Reaction Force3"))
                {
                    A = 0.002;
                    w = 2 \* pi;
                }
...
                // Compute the return value and write
to file
                arrForceTorqueValue[i] = A \* System.Math.Sin(w
\* (Time + deltaT));
...
        public
int motionStudy\_**MotorOutputTimeStepChangeNotify**(double
Time, object MotorNames,
object Position,
object Velocity,
object Acceleration,
object ForceOrTorque,
...
object MotorValue)
        {
            // Fire event for every external motor
output time step change
            int
numMotors = 0;
...
<= numMotors - 1; i++)
            {
                if ((arrMotorNames[i].ToString()
== "RotaryMotor1") | (arrMotorNames[i].ToString()
== "RotaryMotor2"))
                {
                    A = (pi / 4);
                    w = (pi / 2);
...
                //Compute the return value and write
to file
                arrMotorValue[i] = A \* System.Math.Sin(w
\* (Time + deltaT));
...
        public
int motionStudy\_**OutputTimeStepChangeNotify**(double
Time)
        {
...
        public
int motionStudy\_**StartCalculateNotify**(double
Time)
        {
...
System.IO.StreamWriter("c:\\temp\\ForceAndMotorValuesCSharp.txt",
true);
            outputWrite.WriteLine("Recalculation
started...");
            outputWrite.WriteLine("  ");
            outputWrite.Close();
...
        public
int motionStudy\_**StopCalculateNotify**(double
Time)
        {
