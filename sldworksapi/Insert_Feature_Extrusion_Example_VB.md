...
    Set myRefPlane = Part.**FeatureManager**.**InsertRefPlane**(8,
0.01, 0, 0, 0, 0)
    boolstatus = Part.**Extension**.**SelectByID2**("Front
Plane", "PLANE", 0, 0, 0, True, 0, Nothing, 0)
    Set myRefPlane = Part.**FeatureManager**.**InsertRefPlane**(8,
...
    vSkLines = Part.**SketchManager**.**CreateCornerRectangle**(-2.50462141853123E-02,
1.57487558892494E-02, 0, 2.75128867944718E-02, -0.015559011842391, 0)
...
    ' Boss extrusion start condition reference
is Plane2, and the extrusion end is offset 3 mm from the end condition
reference, Plane1
    Set myFeature = Part.**FeatureManager**.**FeatureExtrusion3**(True,
...
True, True, True, swStartSurface, 0, False)
