...
//  5. Type **StateAgeTask\_CSharp** in **Name**.
//  6. Click **OK**.
...
//  9. Click **Show all Files** in the Solution Explorer toolbar and expand
**UserControl1.vb**.
// 10. Replace the code in **Class1****.cs** with this [code](#Class1.cs).
// 11. Replace the code in **UserControl1****.Designer.cs** with this [code](#UserControl1.Designer.cs).
// 12. Replace the code in **UserControl****1.cs**
with this [code](#UserControl1.cs).
// 13. Right-click the project name in the Solution Explorer and click **Add Reference**.
// 14. In the Add Reference dialog:
//     a. Add the SOLIDWORKS PDM Professional
interop assembly as a reference (click **Browse** in the
...
//     a. Navigate to the **bin\Debug**
directory of your built project.
//     b. Click **EPDM.Interop.epdm.dll** and **StateAgeTask\_CSharp.dll**.
//     c. Click **Open**.
//     d. Click **OK**.
...
//     d. Select **Permit** next to the add-in you just
installed.
//     e. Click **OK**.
//  5. In the Administration tool under *vault\_name*, right-click **Tasks** and click **New Task**.
//  6. In the New Task dialog:
//     a. Type **State Age Task** in the upper-left text box.
//     b. Select the **C# Workflow State Age
Task Add-in** from the **Add-in** dropdown.
//     c. Click **Next**.
...
//     g. Click **Next**.
//     h. On the Choose states to check page, select the
file search criteria in:
//        1. **Choose Workflow**.
//        2. **Select States**.
...
namespace StateAgeTask\_CSharp
{
    [Guid(""),
ComVisible(true)]
...
                poCmdMgr.**AddHook**(EdmCmdType.**EdmCmd\_TaskSetupButton**);
                poCmdMgr.**AddHook**(EdmCmdType.**EdmCmd\_TaskRun**);
                poCmdMgr.**AddHook**(EdmCmdType.**EdmCmd\_TaskDetails**);
            }
...
                    case EdmCmdType.**EdmCmd\_TaskSetupButton**:
...
                    props.**TaskFlags** = (int)EdmTaskFlag.**EdmTask\_SupportsScheduling** + (int)EdmTaskFlag.**EdmTask\_SupportsDetails**;
...
                {
                    TaskInstance.**SetStatus**(EdmTaskStatus.**EdmTaskStat\_Running**);
...
                    TaskInstance.**SetStatus**(EdmTaskStatus.**EdmTaskStat\_DoneOK**, 0, "", NotificationArray, ProgresssMsg);
                }
...
                //State strings
                string[]
StatesObj = States.Split(new string[] { "\\n" }, StringSplitOptions.None);
...
                    Search.**SetToken**(EdmSearchToken.**Edmstok\_StateBefore**, dt);
                    Search.**SetToken**(EdmSearchToken.**Edmstok\_StateName**, State);
...
                WorkflowMgr = (IEdmWorkflowMgr6)mVault.**CreateUtility**(EdmUtility.**EdmUtil\_WorkflowMgr**);
...
        private void WorkflowsComboBox\_SelectedIndexChanged(System.Object sender, System.EventArgs e)
        {
            try
            {
...
                string[]
States = SelectedStates.Split(new string[] { "\\n" }, StringSplitOptions.None);
...
            this.WorkflowsComboBox.SelectedIndexChanged += new System.EventHandler(this.WorkflowsComboBox\_SelectedIndexChanged);
            //
            // WorkflowLabel
            //
