...
//    b. Type **ItemRefs\_CSharp** in **Name**.
...
//    structures.
// 4. Ensure that an item folder with at least one item is checked into the
vault.
// 5. Click **Debug > Start Debugging** or press F5.
//
// Postconditions:
// 1. Displays the Get
and set item references dialog box.
// 2. Select a vault view.
// 3. Click **Get
item references**.
//
a. Click an item folder
in the vault.
//    b. Click **OK**.
//    c. Displays a message box confirming your folder selection.
//    d. Click **OK**.
//    e. Displays a message box with item references for the
first item found
//       in the selected folder.
//    f. Click **OK**.
// 4. Click **Set item references**.
//    a. Click one or more files to add as references to the
item.
//    b. Click **Open**.
...
        public void GetItemRefs\_Click(System.Object sender, System.EventArgs e)
        {
...
                folder = (IEdmFolder6)vault2.**BrowseForFolder2**(this.Handle.ToInt32(), "Select a folder", vault2.**RootFolder**, (int)EdmBrowseForFolderFlag.EdmBwsFld\_None);
                if ((folder != null))
                {
                    if (folder.**ObjectType** == EdmObjectType.EdmObject\_Folder)
                    {
...
                search = (IEdmSearch7)vault2.**CreateUtility**(EdmUtility.EdmUtil\_Search);
                search.**FindFiles** = true;
                search.**FindFolders** = false;
                search.**Recursive** = true;
                search.**SetToken**(EdmSearchToken.Edmstok\_FindItems, true);
...
                while ((result != null))
                {
                    if ((result.**IsKindOf**(EdmObjectType.EdmObject\_Item)))
                    {
...
                        item.**GetReferences**((int)EdmRefFlags.EdmRef\_Dynamic + (int)EdmRefFlags.EdmRef\_Static + (int)EdmRefFlags.EdmRef\_Item + (int)EdmRefFlags.EdmRef\_File, out refs);
...
                            if ((refs[idx].**mlEdmRefFlags** & (int)EdmRefFlags.EdmRef\_Dynamic) == (int)EdmRefFlags.EdmRef\_Dynamic)
                            {
                                msg = msg + " (auto-update reference)";
                            }
                            else
                            {
                                if ((refs[idx].**mlEdmRefFlags** & (int)EdmRefFlags.EdmRef\_Static) == (int)EdmRefFlags.EdmRef\_Static)
                                {
...
                item.**GetReferences**((int)EdmRefFlags.EdmRef\_Dynamic + (int)EdmRefFlags.EdmRef\_Static + (int)EdmRefFlags.EdmRef\_Item + (int)EdmRefFlags.EdmRef\_File, out oldRefs);

                //Let the user browse for new file references
                IEdmStrLst5 pathList = vault2.**BrowseForFile**(this.Handle.ToInt32(), (int)EdmBrowseFlag.EdmBws\_ForOpen + (int)EdmBrowseFlag.EdmBws\_PermitVaultFiles + (int)EdmBrowseFlag.EdmBws\_PermitMultipleSel, "All Files|\*.\*||");
...
                    newRefs[idx].**mlEdmRefFlags** = (int)EdmRefFlags.EdmRef\_Static + (int)EdmRefFlags.EdmRef\_File;
...
                //Check in the item
                fileInt.**UnlockFile**(this.Handle.ToInt32(), "Updated item references", (int)EdmUnlockFlag.EdmUnlock\_IgnoreRefsNotLockedByCaller);
...
            this.GetItemRefs.Click +=new System.EventHandler(GetItemRefs\_Click);
            //
            //OpenFileDialog
            //
