...
3. In the **.NET Framework** dropdown at the top of the
   ...
   .NET Framework 4.5 or later. See [Using .NET
...
7. Right-click the project name in the Solution Explorer and click
   **Add > Reference**.
   1. Click **Browse** in the left-hand panel, navigate to and select
...
8. Right-click the project name in the Solution Explorer and
   ...
   3. On the Build tab,
      select **AnyCPU** for the target platform, de-select
      **Prefer 32-bit** and select **Register for COM
      interop**.
9. Make this
   a Debug Add-in:
...
10. If creating this add-in on a 64-bit computer, edit
    *project\_path**\**project\_name\project\_name\**project\_name*.**csproj**
    in Notepad:
...
11. Double-click **Class1.cs** in the Solution Explorer
...
public class Class1 : IEdmAddIn5- To populate the GUID
  attribute above, click **Tools > Create GUID** in the IDE, select GUID
  Format 5, click **Copy**, and click **Exit**. Populate
  [Guid(""),
...
- Implement
  ...
          poCmdMgr.AddCmd(1, "C# Add-in", (int)EdmMenuFlags.EdmMenu\_Nothing);
...
>         if (poCmd.meCmdType == EdmCmdType.EdmCmd\_Menu)
>         {
>             if (poCmd.mlCmdID == 1)
> {
...
- Install
  ...
  4. Click **Add Add-in**, browse to
     ...
     *project\_path**\**project\_name\project\_name***\bin\Debug**,
     click *project\_name***.dll**, and click **Open**.
...
- Close Notepad.
  ...
  > [Guid(""),
  > ComVisible(true)]
  > // See step 14 above to create the GUID
