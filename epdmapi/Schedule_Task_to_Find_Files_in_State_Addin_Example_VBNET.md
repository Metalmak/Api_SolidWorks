...
'  3. Select **.NET Framework 4.5** or
later in the dropdown.
...
'  9. Create a user
control similar to the one shown above and change:
...
' 11. Replace the code in **Class1****.vb** with this [code](#Class1.vb.2).
' 12. Replace the code in **UserControl1****.Designer.vb** with this [code](#UserControl1.Designer.vb).
' 13. Replace the code in **UserControl1****.vb**
with this [code](#UserControl1.vb).
...
'     and log in as Admin.
'  2. Under
*vault\_name*, right-click **Add-ins** and click **New Add-in**.
'     a. Navigate to the
...
'     d. Click **OK**.
'  3. Click **OK** after
reading the SOLIDWORKS PDM Professional warning dialog.
'  4. In the taskbar notification area:
'     a. Click the SOLIDWORKS PDM Professional icon.
'     b. Click **Task Host Configuration**.
'     c. Select *vault\_name* in the **File Vault**
dropdown.
'     d. Select **Permit** next to the add-in you just
installed.
'     e. Click **OK**.
'  5. In the Administration tool under *vault\_name,*
right-click **Tasks** and click **New Task**.
'  6. In the New task dialog:
'     a. Type **State
Age Task** in the upper-left text box.
'     b. Select the **VB.NET Workflow
State Age Task Add-in** from the **Add-in** dropdown.
'     c. Click **Next**.
'     d. On the Execution Method properties page, select the
local computer.
'     e. Click **Next**.
...
search criteria in: **'**        1. **Choose Workflow**.
'        2. **Select States**.
...
'        3. In the **Text** field, type **The task successfully completed at**,
type a space,
...
logged-in user receives a notification
'     with the list of files that have been in
the specified workflow states a
...
        EdmUtility.**EdmUtil\_WorkflowMgr**)
...
  Private Sub WorkflowsComboBox\_SelectedIndexChanged( \_
...
                props.**TaskFlags** = \_
                  EdmTaskFlag.**EdmTask\_SupportsScheduling** + \_
                  EdmTaskFlag.**EdmTask\_SupportsDetails**
...
                  EdmTaskStatus.**EdmTaskStat\_Running**)
...
                TaskInstance.**SetStatus**( \_
                  EdmTaskStatus.**EdmTaskStat\_DoneOK**, 0, "", \_
...
Search.**SetToken**(EdmSearchToken.**Edmstok\_AllVersions**,
True)
                  Search.**SetToken**( \_
                  EdmSearchToken.**Edmstok\_StateBefore**, dt)
                Search.**SetToken**( \_
                  EdmSearchToken.**Edmstok\_StateName**, State)
