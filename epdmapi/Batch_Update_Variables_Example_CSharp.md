...
//    structures.
// 5.
Ensure that at least one text file is
checked out, and at least one folder
...
variables in the data cards
//          only of checked-out
text files.
//        \* Updates the Description variable
of the data cards of all folders
//          with the word **Folder** in
their names.
...
        private void BatchUpdateButton\_Click(System.Object sender, System.EventArgs e)
        {
            try
            {
...
                Update = (IEdmBatchUpdate2)vault2.**CreateUtility**(EdmUtility.EdmUtil\_BatchUpdate);

                //Get the IDs of the file and folder
card variables to update
                int TitleID = 0;
                int CommentID = 0;
                int DescriptionID = 0;
...
                //Search for all text files in the
vault
                IEdmSearch5 Search = default(IEdmSearch5);
                Search = (IEdmSearch5)vault2.**CreateUtility**(EdmUtility.EdmUtil\_Search);
...
                while ((Result != null))
                {
                    Update.**SetVar**(Result.**ID**, TitleID, "My Title", "", (int)EdmBatchFlags.EdmBatch\_Nothing);
                    Update.**SetVar**(Result.**ID**, CommentID, "My Comment", "", (int)EdmBatchFlags.EdmBatch\_Nothing);
                    Result = Search.**GetNextResult**();
                }

//Search for all folders whose names contain
"Folder" in the vault
...
                while ((Result != null))
                {
                    Update.**SetFolderVar**(Result.**ID**, DescriptionID, "My Description", (int)EdmBatchFlags.EdmBatch\_Nothing);
                    Result = Search.**GetNextResult**();
                }

                //Write all the card
variable values to the database
...
            this.BatchUpdateButton.Click += new System.EventHandler(this.BatchUpdateButton\_Click);
            //
            //VaultsComboBox
            //
