...
//    for *drawing\_document\_with\_external\_references*.
...
        const
string
sDocFileName = *"drawing\_document\_with\_external\_references"**;*        SwDMClassFactory
swClassFact = default(SwDMClassFactory);
        SwDMApplication
swDocMgr = default(SwDMApplication);
        SwDMDocument23
swDoc = default(SwDMDocument23);
        SwDMSearchOption
...
SwDMDocumentLicenseType\_e licenseType = 0;

        // Determine type of SOLIDWORKS
file based on file extension

        if
(sDocFileName.EndsWith("slddrw"))
        {
...
        swDoc = (SwDMDocument23)swDocMgr.**GetDocument**(sDocFileName,
nDocType, false,
out nRetVal);
...
licenseType = (SwDMDocumentLicenseType\_e)swDoc.**GetLicenseType**();

        Debug.Print("License type as defined in SwDMDocumentLicenseType\_e: " + licenseType);
...
        vDependArr = (string[])swDoc.**GetAllExternalReferences5**(swSearchOpt,
out
vBrokenRefs, out
vIsVirtuals, out
vTimeStamps, out
...
        foreach (string isImported\_loopVariable in (string[])vIsImported)
        {
            Debug.Print("  Imported component:  " + isImported\_loopVariable);
        }
    }
}