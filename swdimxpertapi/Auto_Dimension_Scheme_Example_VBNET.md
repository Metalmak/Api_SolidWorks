...
'---------------------------------------------------------------------------
' Preconditions:
'  1. Open *public\_documents***\samples\tutorial\dimxpert\bracket\_auto\_plusminus.sldprt.**
...
F5 and observe that tolerance status is turned off.
...
F5 and observe that all tolerance annotations have been
'     removed from
the part.
'
' Postconditions: None
...
        ' Get the default DimXpert schema
using IModelDocExtension.DimXpertManager()
        swSchema =
swModelDocExt.DimXpertManager("Default",
True)
...
            If
Not (IsNothing(appliedAnnotations))
Then
                For
p = 0 To
