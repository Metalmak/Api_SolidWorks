...
' 1. Open
*public\_documents*\samples\tutorial\api\cover\_with\_geometric\_tolerances.sldprt.

' 2.
Click **Geometric Tolerance** the DimXpert toolbar.

'    In
the Geometric Tolerance Properties dialog:

'    a.
Select **Cylindricity** from the **Symbol** dropdown.

'    b.
In the second row, select **Profile of Surface** from the **Symbol** dropdown

'       and type a tolerance of 0.5 next to it.

'    c.
Click the outer base cylinder and click again outside the part
...
'    Flatness1,
Circularity1, Diameter1, CounterSinkAngle1,
...
interop assembly is loaded

'    (right-click
the project in Project Explorer and click on **Add Reference >**
...
' 1. Writes the output to
c:\temp\dimXpertInfo.txt.
...
        ElseIf
annoType = swDimXpertAnnotationType\_e.swDimXpertGeoTol\_Flatness
Then
...
        ElseIf
annoType = swDimXpertAnnotationType\_e.swDimXpertGeoTol\_Circularity
Then
...
        ElseIf
annoType = swDimXpertAnnotationType\_e.swDimXpertGeoTol\_Cylindricity
Then
...
        boolstatus
= annotation.GetPerUnitArea(enabled,
length, width, I, J, K)
...
            Dim
csDiameter As IDimXpertCounterSinkDiameterDimTol

            csDiameter
= annotation

            If
csDiameter.ReferenceFeature Is
Nothing Then
...
                strs.Add("Reference
Feature: " + csDiameter.ReferenceFeature.Name)
...
            Dim
csAngle As IDimXpertCounterSinkAngleDimTol
...
            If
csAngle.ReferenceFeature Is Nothing
Then
...
                strs.Add("Reference
Feature: " + csAngle.ReferenceFeature.Name)
